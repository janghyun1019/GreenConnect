...
function ModifyPostDetail() {
    const navigate = useNavigate();
    const { postId } = useParams(); // 수정하는 postId 받아옴
    ...
    const [postDetail, setPostDetail] = useState(null); // 수정전내용 불러올 용도
    // 상태 관리
    ...
    const handleSubmit = (e) => {
        ...
        if (isConfirmed) {
            ...
            // 백엔드 API로 데이터 전송
            axios.post('/api/modifyWritePost', formData)
                .then((response) => {
    ...
    useEffect(() => {
        if (postDetail) {
            // postDetail에서 값을 가져와 상태로 설정
            setTitle(postDetail.postTitle || '');
            setProductType(postDetail.postProductType || '');
            setContent(postDetail.postContent || '');
            setSalesUnit(postDetail.postSalesUnit || '');
            setPrice(postDetail.postPrice || '');
            setSpot(postDetail.postSpot || '');
            setCost(postDetail.postCost || '');
            setStoreId(postDetail.storeId || '');
        }
    }, [postDetail]);


    if (loading) return <div>로딩 중...</div>; // postDetail 정보 완전히 가져올때까지 대기
    return (
        <div className="writePostContainer">
            <div className="postMainTitle">판매글 수정</div>

            {/* 작성자 정보 */}
            <div className="userInfoBox">
                <div>작성자: {postDetail.nickName}</div>
            ...
            <form id="writePostForm" action="" method="post" onSubmit={handleSubmit}>
                ...
                {user && (
                    <>
                        ...
                        <input type="hidden" name="nickName" value={postDetail.nickName} />
                    </>
                )}
                ...
                <button className='saveBtn' type='submit'>수정</button>
...
export default ModifyPostDetail;